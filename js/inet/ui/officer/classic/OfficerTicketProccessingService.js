// #PACKAGE: officer-ticket-processing
// #MODULE: OficerTicketProcessingService
$(function () {
  var notify = null;
  ...
  var createTicketReturnWidget = function (parent) {
    if (!ticketReturnWidget) {
      ticketReturnWidget = new iNet.ui.onegate.receiver.TicketReturnWidget({
        allowSave: false,
        urlReturn: iNet.getXUrl('onegate/office/ticketstatus')
      });
      ...
      mainWidget.on('transferesb', function(wg, ids, data){
        ...
        if(mainWidget) {
          ...
          if(__data.type == 'ERROR') {
            __notify.setType('error');
            __notify.setContent(iNet.resources.onegate.ticket.esb_send_error);
          }
          else {
            var grid = mainWidget.getGrid();
            if(grid) {
              for (var i = 0; i < ids.length; i++) {
                var __id = ids[i] || {};
                var __record= grid.getById(__id);
                __record.status= 'SUBMITED';
                grid.update(__record);
              }
              grid.commit();
            }
            __notify.setType('success');
            __notify.setContent(iNet.resources.onegate.ticket.esb_send_success);
          }
          __notify.show();
        }
      });
