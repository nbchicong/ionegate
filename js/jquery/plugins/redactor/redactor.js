...
(function($){
	...
	Redactor.prototype = {
		...
		{	
			...
			// paste
			var oldsafari = false;
			if (iNet.isWebkit && navigator.userAgent.indexOf('Chrome') === -1) 
			{
				var arr = iNet.browserVersion.split('.');
		...
		keyup: function()
		{
			this.$editor.keyup($.proxy(function(e)
			{
				...
				{
					if (iNet.isWebkit && this.opts.paragraphy)
					{
		...
		keydown: function()
		{
			this.$editor.keydown($.proxy(function(e)
			{
				...
				// breakline
				if (this.opts.paragraphy === false && iNet.isWebkit && key === 13 && !e.shiftKey && !e.ctrlKey && !e.metaKey)
				{
					e.preventDefault();			
		...
		execCommand: function(cmd, param)
		{
			...
			try
			{
				...
				else if (cmd === 'formatblock' && iNet.isIE)
				{
		...
		saveSelection: function()
		{
			...
			if (iNet.isIE && parseInt(iNet.browserVersion, 10) < 9)
			{
				var node = this.$editor.get(0);
		...
		fileUploadCallback: function(json)
		{
			...
			if (json !== false)
			{
				...
				// chrome fix
				if (iNet.isWebkit && !!window.chrome)
				{
					link = link + '&nbsp;'; 
				}
